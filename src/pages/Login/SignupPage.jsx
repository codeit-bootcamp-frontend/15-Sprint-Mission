...
function SignupPage() {
  ...
  const passwordCheck = usePasswordInput();

  const [nickname, setNickname] = useState("");
  const [nicknameTouched, setNicknameTouched] = useState(false);
  const nicknameError = nicknameTouched && nickname.trim().length === 0;

  const passwordsMismatch =
    passwordCheck.touched && passwordCheck.value !== password.value;
          ...
          <div className="nickname-container">
            ...
            <div className="nickname-input">
              <input
                ...
                className={nicknameError ? "warning" : ""}
          ...
          <div className="password-check-container">
            ...
            <div className="password-input">
              <input
                ...
                type={passwordCheck.visible ? "text" : "password"}
                value={passwordCheck.value}
                onChange={passwordCheck.onChange}
                onBlur={passwordCheck.onBlur}
                className={passwordsMismatch ? "warning" : ""}
                placeholder="비밀번호를 다시 입력해주세요"
              />
              <button
                type="button"
                className={`visibility ${passwordCheck.visible && "visible"}`}
                onClick={passwordCheck.toggleVisible}
              />
              {passwordsMismatch && (
